...
import com.codejames.registerlogin.util.util.UuidUtils;
...
public class SystemLogAspect {
	...
	private static final ThreadLocal<UserDetails> currentUser=new NamedThreadLocal<>("ThreadLocal user");
...
/*	@Pointcut("@annotation(com.myron.ims.annotation.SystemServiceLog)")
	 ...
	 * 方法规则拦截
	 */
	@Pointcut("execution(* com.codejames.registerlogin.controller.*.*(..))")
	...
	public void doBefore(JoinPoint joinPoint) throws InterruptedException{
		...
		UserDetails user = (UserDetails) session.getAttribute("username");
		currentUser.set(user);
		logger.info("user: {}",currentUser.get());
...
//        if(user == null){
//    		HttpSession session = request.getSession();
//    	    user = (UserDetails) session.getAttribute("username");
//    	    if(user==null){
//    	    	return;
//    	    }
//        }
